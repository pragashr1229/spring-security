...
public class SecurityConfigurer extends WebSecurityConfigurerAdapter {
	...
	// this method is mainly to configure our own user name & password
		...
		protected void configure(AuthenticationManagerBuilder auth) throws Exception {
			...
			/*
			auth.inMemoryAuthentication().withUser("abc").password("{noop}xyz").roles("ADMIN").and()
			.withUser("aaa").password("{noop}xyz").roles("ADMIN");
			*/
			...
			/*auth.inMemoryAuthentication().withUser("abc").password("xyz").roles("ADMIN").and()
			.withUser("aaa").password("xy0z").roles("ADMIN");*/
			
			/*auth.inMemoryAuthentication().
				withUser("abc")
				.password("{noop}xyz") // by default spring security accepts encrypted password, when ever we are providing the plain text will mention {noop}
				//.password("xyz")
				.password(getPasswordEncoder().encode("xyz"))
				.roles("ADMIN") // in case of spring security ever user should have a role otherwise user is invalid
				.and()
				.withUser("test")
				.password("{noop}hello")
				//.password("hello")
		...
		// ANother way to specify that password is not encrypted to spring security framework using creating a bean as below
		/*
		@Bean
